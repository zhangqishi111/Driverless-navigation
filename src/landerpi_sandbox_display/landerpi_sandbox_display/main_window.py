...
class MainWindow(QMainWindow):
    def __init__(
        ...
        robot_layout = self.robot_state_panel.layout()

        self.robot_x_value = self._add_value_row(
            robot_layout,
            'X',
        )

        self.robot_y_value = self._add_value_row(
            robot_layout,
            'Y',
        )

        self.robot_yaw_value = self._add_value_row(
            robot_layout,
            'Yaw',
        )
        ...
        self.goal_x_value = self._add_value_row(
            navigation_layout,
            'Goal X',
        )
        ...
        self.local_path_value = self._add_value_row(
            navigation_layout,
            'Local Path',
            '0 points',
        )

        self.actual_path_value = self._add_value_row(
            navigation_layout,
            'Actual Path',
            '0 points',
        )
        ...
        self.map_frame_value = self._add_value_row(
            map_info_layout,
            'Frame',
        )

        self.map_resolution_value = self._add_value_row(
            map_info_layout,
            'Resolution',
        )

        self.map_size_value = self._add_value_row(
            map_info_layout,
            'Map Size',
        )

        self.map_origin_value = self._add_value_row(
            map_info_layout,
            'Origin',
        )

        self.event_log_panel = self._create_panel(
            'Event Log'
        )
